...
%MK2_TRI_2DF Makes two triangular meshes by using boundary line data
%        from a two-dimensional digitized MRI slice.  The first line is
%        assumed to be cartilage and the second line is assumed to be
%        bone.  The region is divided into two regions using a midline
%        between the two lines.
%
%        [TRI1,TRI2,XY] = MK2_TRI_2DF(DAT) given a cell array
...
%        coordinates in a two columns matrix XY.
%
%        [TRI1,TRI2,XY] = MK2_TRI_2DF(DAT,DIST) midline points must
%        be within a distance DIST of the second line.  The default
%        value is Inf.  See midline.m.
%
%        [TRI1,TRI2,XY] = MK2_TRI_2DF(DAT,DIST,SCAL) a one or two
...
%
%                4.   M-files InterX.m, midline.m, near2.m, lsect2.m and
%                lsect2a.m must be in the current directory or path.
%
%                5.  Similar to mk2_tri_2d.m except the slopes of the
...
%
if (nargin<1)
  error(' *** ERROR in mk2_tri_2df:  No input data!');
end
%
if nargin<2
...
%
if nslice~=2
  error([' *** ERROR in mk2_tri_2df:  Input cell array must ', ...
...
if dotp<tol
  ...
  if dotp2<dotp         % Revert back to original ordering
    warning([' *** WARNING in mk2_tri_2df:  Ordering of points', ...
...
if isempty(id2)||id2<npts2-npts2/1.5
  id2 = npts2-1;
end
%
idc = id1:id2+1;
