%#######################################################################
%
%                 * MRI FIT PTOA Slice Visit Program *
%
...
%     mri_fitpsv.xlsx, in the "Results\mri_fitpsv" directory.
%
...
%
%     18-Mar-2025 * Mack Gardner-Morse
%
...
%
resdir = fullfile('Results','mri_fitpsv');       % Results directory
%
ifirst = true;          % First write to file
% ifirst = false;         % First write to file
xlsnam = 'mri_fitpsv.xlsx';            % Results spreadsheet
...
hdrs2 = {'Pixels' 'T1R/T2S' 'RSS' 'ValidPix' 'Mean' 'Min' 'Max' ...
         'SD' 'COV'};
%
psnam = fullfile(resdir,'mri_fitpsv_');          % Start of PS file name
...
%
if ~ifirst
  load(fullfile(resdir,'mri_fitpsv.mat'));
end
%
...
%
        ...
        roinams = {d.name}';
        idr = contains(roinams,'_prois','IgnoreCase',true);     % Masks
%
        rhonams = roinams(~idr);
...
%
        if nrho~=nroi
          error([' *** ERROR in mri_fitpsv:  Number of T1rho MAT', ...
...
%
      if nstar~=nroi
        error([' *** ERROR in mri_fitpsv:  Number of T2* MAT files', ...
...
% Save to MAT File
%
save(fullfile(resdir,'mri_fitpsv.mat'),'t1r_res','t1r_npx', ...
