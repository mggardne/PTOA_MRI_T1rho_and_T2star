function roi = rd_roi7hp(filenam,ipx)
%RD_ROI7HP  Reads an OSIRIX ROI CSV file.
%
%         ROI = RD_ROI7HP(FILENAM) reads the OSIRIX ROI CSV file,
...
%
%         ROI = RD_ROI7HP(FILENAM,IPX) if IPX is true (nonzero) the X
...
%         and the field "area" with the area of each ROI on each slice
%         in cm^2.
%
...
%
%         25-Sep-2024 * Mack Gardner-Morse
%
...
%
if (nargin<1)
  error(' *** ERROR in RD_ROI7HP:  An input file name is required!');
end
%
if (nargin<2)
  ipx = false;
...
%
     for k = 1:npts
        if ipx
          ...
          mat(k,:) = mat(k,:)+0.5;     % Half pixel to match image indexing
        else
