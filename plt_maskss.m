function img = plt_maskss(maskf,maskt,rsl,nrsl,maskfr,rslf,masktr, ...
                          rslt,v,fps,sn,rdir,leg,pnam)
%PLT_MASKSS Plots the cartilage regions of interests for the PTOA knee
%           sagittal segmentations for the femur and tibia.
%
%          PLT_MASKSS(MASKF,MASKT,RSL,NRSL,MASKFR,RSLF,MASKTR,RSLT,V)
...
%                  with spaces.
%
%                  3.  Modified to leave one slice (k==11) plot open.
%
%          03-Oct-2023 * Mack Gardner-Morse 
%
...
%
if (nargin<9)
  error([' *** ERROR in PLT_MASKSS:  At least nine (9) inputs', ...
         ' are required!']);
end
%
...
%
if ~islogical(maskf)
  error([' *** ERROR in PLT_MASKSS:  First input must be a', ...
         ' logical array!']);
end
%
...
cmap = [gmap; jmap];
% cscal = [0.0221 0.3125 0.5078 0.6055 0.7031 0.8984;
%          0.1762 0.3516 0.5469 0.6641 0.7422 0.9766];
cscal = [0.0221 0.3125 0.4878 0.6055 0.7031 0.8984;
         0.1762 0.3516 0.5669 0.6641 0.7422 0.9766];
%
...
%
        m = 3-l;
        img(maskltl) = cscalx(m,6);    % Red/Brown - Medial trochlea
        img(masklcl) = cscalx(m,5);    % Dark Tan/Orange - Medial central
        img(masklpl) = cscalx(m,4);    % Yellow/Light Tan - Medial posterior
        img(maskmtl) = cscalx(m,1);    % Blue/Dark Blue - Lateral trochlea
        img(maskmcl) = cscalx(m,2);    % Light Blue/Cyan - Lateral central
        img(maskmpl) = cscalx(m,3);    % Green/Lime - Lateral posterior   
%
     end
   end
%
