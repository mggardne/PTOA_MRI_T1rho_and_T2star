...
%                  second line.
%
%                  3.  M-files InterX.m, lsect2a.m, lsect3.m, lsect4.m,
%                  and lsect5.m, must be in the current directory or
%                  path.
%
%          20-Aug-2020 * Mack Gardner-Morse
%
%          13-Feb-2023 * Mack Gardner-Morse * Added check of midline
%          to see if it intersects with itself and use the first
%          (cartilage) line instead of the second (bone) line for the
%          perpendiculars to find the midline.
%
...
xymid = [(xyb(bgpt,:)+xyc(1,:))/2; xymid(id1+1:id2-1,:); ...
         (xyb(endpt,:)+xyc(end,:))/2];
%
% Check that Midline Does Not Intersect Itself
%
p = InterX(xymid');
%
if ~isempty(p)          % Try cartilage instead of bone line
%
% Get Slopes of the First (Cartilage) Line
%
  nc = size(xyc,1);     % Number of cartilage points
  ni = nc-2;            % Number of interior points
%
  dbb = diff(xyc);
  dfb = dbb(1:nc-2,:);  % Forward slope
  dbb = dbb(2:nc-1,:);  % Backward slope
  dcb = (dfb+dbb)/2;    % Central slope
%
% Check Slopes
%
...
% Get Perpendiculars
%
  idi = 2:nc-1;         % Index to interior points
  mc = -dcb(:,1)./dcb(:,2);  % Get perpendicular slopes
  xc = xyc(idi,1);
  yc = xyc(idi,2);
  bc = yc-mc.*xc;       % Y-intercept for lines
%
% Plot Points
%
  if iplt
    ...
    %
    xp = [xc-15 xc+15]';
    yp = repmat(mc,1,2)'.*xp+repmat(bc,1,2)';
    plot(xp,yp,'g-');
    axis([min(xyb(:,1))-12 max(xyb(:,1))+12 min(xyb(:,2))-12 ...
          max(xyb(:,2))+12]);
  end
%
% Find Intersections within Tolerance
%
  xyi = zeros(ni,2);
  for k = 1:ni
     xy = lsect2a(mc(k),bc(k),xyb);
     nxy = size(xy,1);
     d = xy-repmat(xyc(k,:),nxy,1);
     d = d.*repmat(pspc,nxy,1);
     d = sum(d.*d,2);
     [dmin,idmin] = min(d);            % Intersection closest to cartilage point
     if dmin<tol        % Within tolerance of cartilage point
       xyi(k,:) = xy(idmin,:);
     else
       xyi(k,:) = [NaN NaN];
     end
...
%
  xymid = (xyi+xyc(idi,:))./2;         % Get midpoint
%
% Check for NaNs
%
  bgpt = 1;
...
% Get End Points
%
  xymid = [(xyc(bgpt,:)+xyb(1,:))/2; xymid(id1+1:id2-1,:); ...
           (xyc(endpt,:)+xyb(end,:))/2];
end
%
% Check that Midline Does Not Intersect Itself
...
%
if ~isempty(p)
  error(' *** ERROR in MIDLINE:  Midline intersects itself!');
end
%
