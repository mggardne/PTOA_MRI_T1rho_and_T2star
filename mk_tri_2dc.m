function [tri,xy,nt] = mk_tri_2dc(dat,clip,dist,tol,iplt)
%MK_TRI_2DC Makes a triangular mesh using boundary line data from
%        a two-dimensional digitized MRI slice.  The first line is
%        assumed to be cartilage and the second line is assumed to be
%        bone.
%
%        [TRI,XY] = MK_TRI_2DC(DAT,CLIP) given a cell array containing
...
%        Y coordinates in a two columns matrix XY.
%
%        [TRI,XY,NT] = MK_TRI_2DC(DAT,CLIP) returns the number of
%        triangles, NT.
%
%        [TRI,XY] = mk_tri_2d(DAT,CLIP,DIST) given a distance, DIST,
...
%
%                5.  Similar to mk_tri_2dfc.m except the slopes of the
...
%
if (nargin<2)
  error(' *** ERROR in mk_tri_2dc:  Two inputs are required!');
end
%
clip = clip(:);
%
...
%
if nslice~=2
  error([' *** ERROR in mk_tri_2dc:  Input cell array must have ', ...
