...
%
   pwd
%
% Check for prois Mat Files
%
   ido = false;
%
   rnams = dir('T*_prois.mat');
%
   if size(rnams,1)<4
%
     ido = true;
%
   else                 % Check for newer segmentation files
%
     rnams = dir('T1rho*_prois.mat');
     rdates = {rnams.date}';
     rdats = datetime(rdates,'InputFormat','dd-MMM-yyyy HH:mm:ss');
%
     snams = dir(fullfile('RHO','0*_ALL_*.csv'));
     sdates = {snams.date}';
     sdats = datetime(sdates,'InputFormat','dd-MMM-yyyy HH:mm:ss');
%
     if size(rdats,1)~=size(sdats,1)
       ido = true;
     elseif any(sdats>rdats)
       ido = true;
     end
%
     rnams = dir('T2star*_prois.mat');
     rdates = {rnams.date}';
     rdats = datetime(rdates,'InputFormat','dd-MMM-yyyy HH:mm:ss');
%
     snams = dir(fullfile('T2S','0*_ALL_*.csv'));
     sdates = {snams.date}';
     sdats = datetime(sdates,'InputFormat','dd-MMM-yyyy HH:mm:ss');
%
     if size(rdats,1)~=size(sdats,1)
